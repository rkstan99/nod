    ...
    try {
        ...
        for (const recipient of recipients) {
            try {
                ...
                if (sendAsImage) {
                    const imageUrl = `data:image/png;base64,${await generateImageFromHTML(letterContent, recipient)}`;
                    mailOptions.html = `<img src="${imageUrl}" alt="Letter Image">`; // Embed image in email
                } else {
                ...
                for (const cid in cidMappings) {
                    const imageUrl = cidMappings[cid];
                    const imageBuffer = await getImageBuffer(imageUrl);
                    attachments.push({ filename: `${cid}.png`, content: imageBuffer, cid: cid });
                }
...
async function generateImageFromHTML(htmlContent, recipient) {
    ...
    let processedHtmlContent = replacePlaceholders(htmlContent, [recipient]);

    // Replace CID mappings in the HTML content
    const cidMappings = require('./cids.json');
    for (const cid in cidMappings) {
        const imageUrl = cidMappings[cid];
        processedHtmlContent = processedHtmlContent.replace(new RegExp(`cid:${cid}`, 'g'), imageUrl);
    }

    const browser = await puppeteer.launch({ executablePath: 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe' });
    const page = await browser.newPage();
    await page.setContent(processedHtmlContent); // Use processed HTML content
